...
contract OfferStation is
    Context,
    AccessControlEnumerable,
    ReentrancyGuard,
    ERC2981
{
    bytes32 public constant OFFERSTATION_ADMIN_ROLE =
        keccak256("OFFERSTATION_ADMIN_ROLE");
    ...
    modifier onlyWhenNotPaused() {
        require(
            paused == false,
            "OfferStation: You cannot offer , it is paused for sometime!"
        );
        _;
    }

    constructor(uint96 _platformFee, string memory _version, bool _paused) {
        _setupRole(OFFERSTATION_ADMIN_ROLE, _msgSender());
        _setRoleAdmin(OFFERSTATION_ADMIN_ROLE, OFFERSTATION_ADMIN_ROLE);
    ...
    function acceptOffer(
        ...
        uint256 payoutForMarketplace = (value * platformFeeBasisPoint) / 1000;
    ...
    function setPause() external onlyRole(OFFERSTATION_ADMIN_ROLE) {
